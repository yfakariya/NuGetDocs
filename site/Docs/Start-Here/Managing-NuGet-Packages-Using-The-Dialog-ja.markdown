...
パッケージをインストールするには、そのパッケージを選択して、**インストール**（**Install**）をクリックします。NuGet は選択されたパッケージと、その依存先のパッケージをインストールします。ソリューションにファイルがコピーされ、場合によってはプロジェクトに参照が追加されたり、プロジェクトの *app.config* ファイルや *web.config* ファイルが更新されたりといった具合です。
